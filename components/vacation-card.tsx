    ...
    <Link href={`/vacation/${vacation.id}`}>
      <Card className="overflow-hidden transition-all hover:shadow-md">
        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-start mb-3">
            <div>
              <h3 className="font-semibold">
                {t(
                  `vacationTypes.${vacation.vacation_type?.name?.toLowerCase()}`,
          ...
          {vacation.notes && (
            <p className="text-sm mt-2 line-clamp-2">{vacation.notes}</p>
          )}
        </CardContent>
        <CardFooter className="p-4 pt-0 flex justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-1">
            ...
            <span>
              {t("vacationDetails.requestedOn", "Requested on")} {formatDate(vacation.created_at)}
            </span>
          </div>
        </CardFooter>
      </Card>

                      ...
                      <div className="flex flex-col space-y-4">
                        {mockData.byType.map((item) => (
                          <div key={item.name} className="flex items-center">
                            ...
                            <div className="flex-1">{t(`vacationTypes.${item.name.toLowerCase()}`, item.name)}</div>
                            <div className="font-medium">{item.value} {t("common.days", "days")}</div>
                  ...
                  <ul className="space-y-4">
                    {mockData.insights.map((insight, index) => (
                      <li key={index} className="flex items-start">
                        ...
                        <span>{t(`insights.insight${index+1}`, insight)}</span>
                      </li>
                    ))}
                  </ul>
                </CardContent>
                  ...
                  <div className="flex h-full items-end gap-2">
                    {mockData.byMonth.map((month) => (
                      <div key={month.name} className="flex-1 flex flex-col items-center gap-2">
                        ...
                        />
                        <span className="text-xs font-medium">{t(`calendar.${month.name.toLowerCase()}`, month.name)}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <Card>
              ...
              <CardContent className="h-80 flex items-center justify-center">
                <p className="text-muted-foreground">
                  {t("insights.trendAnalysisMessage", "More detailed trend analysis will be available after you've used the app for a longer period.")}
                </p>
              </CardContent>
            </Card>
          </TabsContent>
